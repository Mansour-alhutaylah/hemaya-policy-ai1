...
const StatCard = ({ icon: Icon, label, value, color }) => (
  <div className="bg-slate-800 border border-slate-700 rounded-xl p-5 flex items-center gap-4">
    ...
    <div>
      ...
      <p className="text-white text-2xl font-bold mt-0.5" style={{ fontVariantNumeric: 'lining-nums' }}>{value}</p>
    </div>
  </div>
);
      ...
      <tr key={i} className="border-b border-slate-700/50">
        {Array.from({ length: cols }).map((_, j) => (
          <td key={j} className="px-4 py-3">
            <div className="h-4 bg-slate-700 rounded animate-pulse" style={{ width: `${60 + (j * 7) % 30}%` }} />
          </td>
        ))}
      </tr>
    ))}
...
function DashboardSection() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([s, l]) => { setStats(s); setLogs(l.slice(0, 5)); })
  ...
  return (
    <div>
      <SectionHeader title="Admin Dashboard" subtitle="System-wide overview" />

      {/* Stats grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {loading ? (
          Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="bg-slate-800 border border-slate-700 rounded-xl p-5 flex items-center gap-4">
              ...
              <div className="flex-1 space-y-2">
                ...
                <div className="h-7 bg-slate-700 rounded animate-pulse w-1/2" />
              </div>
            </div>
          ))
        ) : stats ? (
          <>
            ...
            <StatCard icon={FileText}     label="Uploaded Policies"    value={stats.uploadedPolicies}                              color="bg-indigo-600" />
            <StatCard icon={CheckCircle2} label="Completed Analyses"   value={stats.completedAnalyses}                             color="bg-emerald-600" />
            <StatCard icon={Clock}        label="Pending Analyses"     value={stats.pendingAnalyses}                               color="bg-yellow-600" />
            <StatCard icon={TrendingUp}   label="Avg Compliance Score" value={`${Number(stats.avgComplianceScore).toFixed(1)}%`}   color="bg-teal-600" />
            <StatCard icon={XCircle}      label="Failed Analyses"      value={stats.failedAnalyses}                                color="bg-red-600" />
          </>
        ) : null}
      </div>
      ...
      <div className="mt-8 bg-slate-800 border border-slate-700 rounded-xl p-5">
        ...
        {loading ? (
          ...
          <div className="space-y-0">
            {logs.map((log, i) => {
              const logStatus = log.action?.toLowerCase().includes('fail') ? 'error'
                : log.action?.toLowerCase().includes('delete') ? 'warning'
                : log.action?.toLowerCase().includes('role') ? 'info'
                : 'success';
              return (
                <div key={log.id || i} className="flex items-center justify-between text-sm py-2.5 border-b border-slate-700/50 last:border-0">
                  <div className="flex items-center gap-3 min-w-0">
                    <StatusPill status={logStatus} label={logStatus} />
                    <span className="text-slate-300 truncate">{log.action}</span>
                  </div>
                  <div className="text-slate-500 text-xs whitespace-nowrap ml-4">{fmtTime(log.timestamp)}</div>
                </div>
              );
            })}
          </div>
    ...
    <div>
                  ...
                  <tr key={u.id} className="border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                    <td className="px-4 py-3">
                      <div>
                        ...
                        <p className="text-slate-600 text-[10px] font-mono">{String(u.id).slice(0, 8)}…</p>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-slate-300">{u.role || 'Regular User'}</td>
                    <td className="px-4 py-3">
                      <StatusPill
        ...
        <DialogContent className="bg-slate-800 border-slate-700 text-white">
          ...
          <div className="space-y-3 py-2">
            <p className="text-slate-400 text-sm">
              Changing role for <strong className="text-white">
      ...
      <Dialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <DialogContent className="bg-slate-800 border-slate-700 text-white">
          ...
          <p className="text-slate-300 py-2">This will permanently delete the user and all their data. This cannot be undone.</p>
          <DialogFooter>
    ...
    <div>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-700 bg-slate-900/50">
                <th className="text-left px-4 py-3 text-slate-400 font-medium">File Name</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-10 text-center text-slate-500">No policies found.</td></tr>
              ) : (
                filtered.map(p => (
                  <tr key={p.id} className="border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-white font-medium truncate max-w-[180px]">{p.file_name}</p>
                          <p className="text-slate-600 text-[10px] font-mono">{String(p.id).slice(0, 8)}…</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-slate-400">{p.department || '—'}</td>
                    <td className="px-4 py-3 text-slate-300 text-xs">{p.uploaded_by || <span className="text-slate-600">Unknown</span>}</td>
                    <td className="px-4 py-3 text-slate-400">{fmt(p.created_at)}</td>
                    <td className="px-4 py-3">
                      {p.framework
                        ? <span className="text-xs bg-slate-700 text-slate-300 px-2 py-1 rounded">{p.framework}</span>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <DialogContent className="bg-slate-800 border-slate-700 text-white">
          ...
          <p className="text-slate-300 py-2">This will permanently delete the policy and all its analysis data.</p>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteTarget(null)}
              className="border-slate-600 text-slate-300 hover:bg-slate-700">Cancel</Button>
    ...
    <div>
        ...
        {loading
          ? Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="bg-slate-800 border border-slate-700 rounded-xl p-5 space-y-3">
                ...
                <div className="h-5 bg-slate-700 rounded animate-pulse w-1/2" />
            ...
            : frameworks.map(fw => {
              ...
              <div key={fw.id} className="bg-slate-800 border border-slate-700 rounded-xl p-5">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  ...
                  <div className="bg-slate-700/50 rounded-lg p-3 text-center">
                    <p className="text-2xl font-bold text-white">{fw.checkpoints}</p>
      ...
      <Dialog open={!!viewFw} onOpenChange={open => !open && setViewFw(null)}>
        <DialogContent className="bg-slate-800 border-slate-700 text-white max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-white text-lg">{viewFw?.name}</DialogTitle>
          </DialogHeader>
          <p className="text-slate-400 text-sm mb-4">{viewFw?.description || 'No description.'}</p>
          {viewLoading ? (
            ...
            <p className="text-slate-500 text-sm">No controls found for this framework.</p>
          ) : (
            <div className="space-y-3">
              {viewDetail.controls.map(ctrl => (
                <div key={ctrl.id} className="bg-slate-900/60 rounded-lg p-3 border border-slate-700">
                  ...
                  {ctrl.checkpoints.length > 0 && (
                    <ul className="mt-2 space-y-1">
                      {ctrl.checkpoints.map(cp => (
                        <li key={cp.id} className="text-slate-400 text-xs flex gap-2">
                          <span className="text-slate-600">{cp.index}.</span>
                          {cp.requirement}
                        </li>
                      ))}
                    </ul>
      ...
      <Dialog open={!!addCtrlFw} onOpenChange={open => !open && setAddCtrlFw(null)}>
        <DialogContent className="bg-slate-800 border-slate-700 text-white max-w-md">
          <DialogHeader>
            <DialogTitle className="text-white">Add Control to {addCtrlFw?.name}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div>
              <label className="text-slate-400 text-xs mb-1 block">Control ID / Code *</label>
              <Input
            ...
            <div>
              <label className="text-slate-400 text-xs mb-1 block">Control Title *</label>
              <Input
            ...
            <div>
              <label className="text-slate-400 text-xs mb-1 block">Severity if Missing</label>
              <Select
          ...
          <DialogFooter>
            <Button variant="ghost" className="text-slate-400" onClick={() => setAddCtrlFw(null)}>Cancel</Button>
            <Button
              className="bg-emerald-600 hover:bg-emerald-700 text-white"
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-700 bg-slate-900/50">
                <th className="text-left px-4 py-3 text-slate-400 font-medium">Policy</th>
                <th className="text-left px-4 py-3 text-slate-400 font-medium">Uploaded By</th>
                <th className="text-left px-4 py-3 text-slate-400 font-medium">Framework</th>
                <th className="text-left px-4 py-3 text-slate-400 font-medium">Score</th>
                <th className="text-left px-4 py-3 text-slate-400 font-medium hidden md:table-cell">Compliant</th>
                <th className="text-left px-4 py-3 text-slate-400 font-medium hidden md:table-cell">Partial</th>
                <th className="text-left px-4 py-3 text-slate-400 font-medium hidden md:table-cell">Gaps</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={9} className="px-4 py-10 text-center text-slate-500">No analyses found.</td></tr>
              ) : (
                analyses.map(a => {
                  ...
                  return (
                    <tr key={a.id} className="border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                      <td className="px-4 py-3 text-white text-xs max-w-[160px] truncate">{a.policy_name}</td>
                      <td className="px-4 py-3 text-slate-300 text-xs">{a.uploaded_by || <span className="text-slate-600">Unknown</span>}</td>
                      <td className="px-4 py-3">
...
function SettingsSection({ adminUser }) {
  ...
  const numInput = (value, setter) => (
    <Input
      ...
      className="bg-slate-700 border-slate-600 text-white focus-visible:ring-emerald-500"
  ...
  const Toggle = ({ checked, onChange }) => (
    <label className="relative inline-flex items-center cursor-pointer">
      ...
      <div className="w-9 h-5 bg-slate-600 rounded-full peer peer-checked:bg-emerald-600 transition-colors after:absolute after:top-0.5 after:left-0.5 after:bg-white after:w-4 after:h-4 after:rounded-full after:transition-all peer-checked:after:translate-x-4" />
    </label>
  );
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Input defaultValue={ADMIN_EMAIL} disabled className="bg-slate-700 border-slate-600 text-slate-400" />
            </div>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
          ...
          <div className="space-y-3">
            <div>
              <label className="text-slate-400 text-xs block mb-1">LLM Model</label>
            ...
            <div>
              <label className="text-slate-400 text-xs block mb-1">Top-K Retrieval</label>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
          ...
          <div className="space-y-3">
            <div>
              <label className="text-slate-400 text-xs block mb-1">Session Timeout (minutes)</label>
              {numInput(sessionTimeout, setSessionTimeout)}
              <p className="text-slate-600 text-[10px] mt-1">Users are logged out after this many minutes of inactivity. Applied on next login.</p>
            </div>
            <div>
              <label className="text-slate-400 text-xs block mb-1">Max Login Attempts</label>
              {numInput(maxAttempts, setMaxAttempts)}
            </div>
            <div>
              <label className="text-slate-400 text-xs block mb-1">Lockout Duration (minutes)</label>
              {numInput(lockoutDuration, setLockoutDuration)}
              <p className="text-slate-600 text-[10px] mt-1">How long an account stays locked after exceeding max attempts.</p>
            </div>
...
function AdminSidebar({ active, setActive, collapsed, setCollapsed, onLogout }) {
  return (
    <aside className={`fixed left-0 top-0 z-40 h-screen bg-slate-900 border-r border-slate-700/50 text-white flex flex-col transition-all duration-300 ${collapsed ? 'w-20' : 'w-64'}`}>
      <div className="flex items-center gap-3 px-5 py-5 border-b border-slate-700/50">
        ...
        {!collapsed && (
          <div>
            <p className="font-bold text-base leading-tight">Himaya Admin</p>
            ...
            return (
              <li key={item.id}>
                <button onClick={() => setActive(item.id)}
                  ...
                  <Icon className={`w-5 h-5 flex-shrink-0 transition-colors ${isActive ? 'text-emerald-400' : 'text-slate-500 group-hover:text-emerald-400'}`} />
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-center">
        ...
        <a href="/" className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-lg text-sm font-medium transition-colors">
          Return to Dashboard
        </a>
      </div>
    </div>
...
export default function Admin() {
  ...
  // Force dark mode on the HTML element while admin panel is mounted.
  // ThemeContext defaults to "light" for regular users; the admin page is
  // always dark regardless of the user's personal theme preference.
  useEffect(() => {
    document.documentElement.classList.add('dark');
    return () => {
      // Restore whatever the user had set when they leave the admin panel
      ...
      <div className={`transition-all duration-300 ${collapsed ? 'ml-20' : 'ml-64'}`}>
        ...
        <header className="bg-slate-900 border-b border-slate-700/50 px-6 py-4 flex items-center justify-between sticky top-0 z-30">
          <div>
            ...
            <p className="text-slate-500 text-xs">Himaya AI — Admin Panel</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-right">
              ...
              <p className="text-slate-500 text-xs">{ADMIN_EMAIL}</p>
            </div>
